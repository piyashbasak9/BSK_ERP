...
class DailyCollectionSheetUpdateView(LoginRequiredMixin, PermissionRequiredMixin, UpdateView):
    ...
    def get_object(self, queryset=None):
        ...
        if obj.verified:
            raise PermissionError("Cannot edit a verified sheet")
        return obj


class DailyCollectionSheetVerifyView(LoginRequiredMixin, PermissionRequiredMixin, View):
    permission_required = 'collections.can_verify_collectionsheet'

    def post(self, request, pk):
        sheet = get_object_or_404(DailyCollectionSheet, pk=pk)
        if not request.user.is_superuser and sheet.branch != request.user.branch:
            return JsonResponse({'error': 'Access denied'}, status=403)
        if sheet.verified:
            return JsonResponse({'error': 'Sheet already verified'}, status=400)

        sheet.verified = True
        sheet.verified_by = request.user
        sheet.verified_at = timezone.now()
        sheet.save()
        return JsonResponse({'success': True, 'message': 'Sheet verified successfully'})


class DailyCollectionSheetPrintView(LoginRequiredMixin, PermissionRequiredMixin, View):
    permission_required = 'collections.view_dailycollectionsheet'
    ...
    def get(self, request, pk):
        ...
        entries = CollectionEntry.objects.filter(sheet=sheet).select_related('member')
        html_string = render_to_string('collections/sheet_pdf.html', {
            'sheet': sheet,
            'entries': entries,
            'today': timezone.now().date(),
        })
        response = HttpResponse(content_type='application/pdf')
        response['Content-Disposition'] = f'filename=collection_sheet_{sheet.id}.pdf'
...
class CollectionEntryCreateView(LoginRequiredMixin, PermissionRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        if sheet.verified:
            form.add_error('sheet', 'Cannot add entry to a verified sheet')
            return self.form_invalid(form)
        return super().form_valid(form)


class CollectionEntryUpdateView(LoginRequiredMixin, PermissionRequiredMixin, UpdateView):
    model = CollectionEntry
    form_class = CollectionEntryForm
    template_name = 'collections/entry_form.html'
    success_url = reverse_lazy('collections_entry_list')
    permission_required = 'collections.change_collectionentry'

    def get_object(self, queryset=None):
        obj = super().get_object(queryset)
        if obj.sheet.verified:
            raise PermissionError("Cannot edit entry of a verified sheet")
        return obj


class CollectionEntryDeleteView(LoginRequiredMixin, PermissionRequiredMixin, DeleteView):
    model = CollectionEntry
    template_name = 'collections/entry_confirm_delete.html'
    success_url = reverse_lazy('collections_entry_list')
    permission_required = 'collections.delete_collectionentry'

    def get_object(self, queryset=None):
        obj = super().get_object(queryset)
        if obj.sheet.verified:
            raise PermissionError("Cannot delete entry of a verified sheet")
        return obj